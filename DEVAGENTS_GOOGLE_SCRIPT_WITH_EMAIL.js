...
function uploadPaymentScreenshotToDrive_(folderId, dataUrl, entryNumber) {
  try {
    ...
    var ext = mimeType.indexOf("png") !== -1
      ? "png"
...
function buildQrCodeFormula_(value) {
  var qrUrl = "https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=" + encodeURIComponent(value);
  return '=IMAGE("' + qrUrl + '")';
}
...
function sendRegistrationReceivedEmail_(adminEmail, opts) {
  ...
  var html = '<div style="font-family: Arial, sans-serif; max-width: 680px; margin: 0 auto; padding: 18px; color: #0f172a;">'
